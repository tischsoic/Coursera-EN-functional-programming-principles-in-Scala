...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    val thisWithoutMostRetweeted = remove(mostRetweeted)
    new Cons(mostRetweeted, thisWithoutMostRetweeted.descendingByRetweet)
  }
...
object GoogleVsApple {
  ...
  def tweetsFilter(l: List[String])(t: Tweet): Boolean =
    l.foldLeft(false)((Encountered, str) => Encountered || t.text.contains(str))

  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweetsFilter(google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweetsFilter(apple))
  /**
